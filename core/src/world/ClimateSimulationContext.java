 ...
 */
public class ClimateSimulationContext implements IClimateSimulationContext {
	/**
...
//	private static final Logger logger = LoggerFactory.getLogger(ClimateSimulator.class);
	...
	public ClimateSimulationContext(float[][] heightmap, int size) {
			...
			for (int width = 0; width < size; width++) {
				for (int height = 0; height < size; height++) {
					...
					if (distArr[width][height] == size) { // Block could update
						...
						if (distArr[posW][height]
								+ (heightmap[posW][height] - currHeight) * heightInfluence <= currentDistance
								|| distArr[width][posH]
										+ (heightmap[width][posH] - currHeight) * heightInfluence <= currentDistance
								|| distArr[negW][height]
										+ (heightmap[negW][height] - currHeight) * heightInfluence <= currentDistance
								|| distArr[width][negH]
										+ (heightmap[width][negH] - currHeight) * heightInfluence <= currentDistance) {
							// Updates over an edge
							distArr[width][height] = currentDistance + 1;
						} else if (distArr[posW][posH]
								+ (heightmap[posW][posH] - currHeight) * heightInfluence <= currentDistance + 0.41421
								|| distArr[negW][posH] + (heightmap[negW][posH] - currHeight)
										* heightInfluence <= currentDistance + 0.41421
								|| distArr[posW][negH] + (heightmap[posW][negH] - currHeight)
										* heightInfluence <= currentDistance + 0.41421
								|| distArr[negW][negH] + (heightmap[negW][negH] - currHeight)
										* heightInfluence <= currentDistance + 0.41421) {
